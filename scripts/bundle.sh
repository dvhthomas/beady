#!/usr/bin/env bash
# Builds the SwiftPM executable and wraps it in build/BeadsViewer.app.
...
STAGING="$ROOT/build/BeadsViewer.app.staging"

swift build --package-path "$ROOT" -c "$CONFIG" --scratch-path "$SCRATCH" --product BeadsViewer
BIN_DIR="$(swift build --package-path "$ROOT" -c "$CONFIG" --scratch-path "$SCRATCH" --show-bin-path)"

rm -rf "$STAGING"
mkdir -p "$STAGING/Contents/MacOS" "$STAGING/Contents/Resources"
cp "$BIN_DIR/BeadsViewer" "$STAGING/Contents/MacOS/BeadsViewer"
...
<plist version="1.0">
<dict>
  <key>CFBundleName</key><string>Beads Viewer</string>
  <key>CFBundleDisplayName</key><string>Beads Viewer</string>
  <key>CFBundleIdentifier</key><string>app.bitsbyd.beads-viewer</string>
  <key>CFBundleExecutable</key><string>BeadsViewer</string>
